...
module.exports = class Okaeri {
    ...
    async queryAccounts(filters = {}, order = false, page = 1){
    ...
    async queryGroups(filters = {}, order = false, page = 1){
    ...
    async isAccountInAnyGroup(accId, groups){
        let result = await this.mongo.group.find({
            code: { $in: groups },
            accounts: this.mongo.id(accId)
        });

        return { ok: Boolean(await result.count()) };
    }
